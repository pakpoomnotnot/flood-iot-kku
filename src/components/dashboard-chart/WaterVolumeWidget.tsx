...
const WaterVolumeWidget = () => {
  // ข้อมูลจำลอง
  const percentage = 71;
  const waterSource = 'บึงหนองโคตร'; // รวมชื่อเป็นคำเดียวเพื่อลดการขึ้นบรรทัดใหม่
  ...
  const DamWaterIcon = () => (
    <div className="flex items-center justify-center relative w-7 h-7"> 
      ...
      <FaWater 
        className="text-[#00b0f0] w-4 h-4 absolute left-[6px] opacity-80" 
      />
    </div>
  );
    ...
    <div className="flex w-full h-auto items-center justify-center rounded-lg border border-none p-0 bg-white">
      ...
      <div className="flex flex-col w-full text-xs items-center p-0.5"> 
        ...
        <div className="flex flex-row items-center justify-center w-full mb-0">
          <DamWaterIcon />
          <span className="text-2xl font-bold text-[#00b0f0] ml-0.5"> {/* ลด ml-1 เป็น ml-0.5 */}
            {percentage}%
          </span>
        </div>
...
export default WaterVolumeWidget;