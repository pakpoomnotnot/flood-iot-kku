...
/* ----------------------------
   ...
   วิกฤต=แดง | เตือนภัย=ส้ม | เฝ้าระวัง=เหลือง | ปกติ=เขียว | ไม่มีข้อมูล=เทา
   ---------------------------- */
const COLOR = {
  critical: "#d32f2f",  // แดง — วิกฤต / น้ำล้นตลิ่ง / ฝนหนักมาก
  warning:  "#ef6c00",  // ส้ม  — เตือนภัย / น้ำมาก / ฝนหนัก
  watch:    "#fbc02d",  // เหลือง — เฝ้าระวัง / น้ำน้อย / ฝนปานกลาง
  normal:   "#2e7d32",  // เขียว — ปกติ / น้ำปกติ / ฝนเล็กน้อย
  nodata:   "#9e9e9e",  // เทา — ไม่มีข้อมูล / ไม่มีฝน
};

// alias map เพื่อให้ใช้ชื่อไทยได้เหมือนเดิม
const pieColors: Record<string, string> = {
  // น้ำในบึง
  "น้ำล้นตลิ่ง":   COLOR.critical,
  "น้ำมาก":        COLOR.warning,
  "น้ำปกติ":       COLOR.normal,
  "น้ำน้อย":       COLOR.watch,
  "น้ำน้อยวิกฤติ": COLOR.critical,  // วิกฤตเช่นกัน ใช้แดง
  // ฝน
  "ฝนตกหนักมาก":  COLOR.critical,
  "ฝนตกหนัก":     COLOR.warning,
  "ฝนตกปานกลาง":  COLOR.watch,
  "ฝนตกเล็กน้อย": COLOR.normal,
  // ท่อ / ถนน
  "วิกฤต":        COLOR.critical,
  "แจ้งเตือน":    COLOR.warning,
  "เฝ้าระวัง":    COLOR.watch,
  "ปกติ":         COLOR.normal,
};
...
const WaterIcon = ({ level }: { level: number }) => {
  ...
  if (level === 3) return <Waves size={size} color={COLOR.normal} strokeWidth={2.5} />;
  if (level === 2) return <Waves size={size} color={COLOR.watch} strokeWidth={2.5} />;
  if (level === 1) return (
    <div className="flex items-center gap-0">
      <Waves size={size} color={COLOR.critical} strokeWidth={2.5} />
      <Waves size={size} color={COLOR.critical} strokeWidth={2.5} className="-ml-2" />
...
const RainIcon = ({ level }: { level: number }) => {
  const size = 16;
  // 4=หนักมาก(วิกฤต)แดง 3=หนัก(เตือน)ส้ม 2=ปานกลาง(เฝ้าระวัง)เหลือง 1=เล็กน้อย(ปกติ)เขียว 0=ไม่มีฝน เทา
  if (level === 4) return (
    <div className="flex items-center gap-0">
...
/* ----------------------------
   Status Icon for pipe/road
   ---------------------------- */
const StatusIcon = ({ level }: { level: number }) => {
  const size = 16;
  // 5=วิกฤต แดง, 4=เตือนภัย ส้ม, 3=เฝ้าระวัง เหลือง, 2=ปกติ เขียว, 0=ไม่มีข้อมูล เทา
  if (level === 5) return (
    <div className="flex items-center gap-0">
      <Waves size={size} color={COLOR.critical} strokeWidth={2.5} />
      <Waves size={size} color={COLOR.critical} strokeWidth={2.5} className="-ml-2" />
...
/* ----------------------------
   Rain Card with Tabs (รายวัน / รายชั่วโมง)
   ---------------------------- */
const RainCard = ({
  title,
  totalStations,
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-[#ead0c7] bg-white p-1 shadow-sm">
      <p className="text-center text-xs font-semibold text-[#2c120c]">{title}</p>
      {/* <p className="text-center text-[10px] text-[#8a6458]">{totalStations} สถานี</p> */}
...
function bucketPondValues(latestRow: Record<string, any>) {
  const stats = { overflow: 0, high: 0, normal: 0, low: 0, criticalLow: 0, noData: 0 };
  Object.entries(latestRow).forEach(([k, v]) => {
    ...
    else if (val > 1.0) stats.overflow++;
...
/** Bucket rain values into categories */
...
export default function Dashboard() {
  ...
  const fetchPond = async () => {
    try {
      ...
      if (json?.status === "success" && Array.isArray(json.rows) && json.rows.length > 0) {
        ...
        setPondChart([
          { name: "วิกฤต",       value: stats.overflow + stats.criticalLow, color: COLOR.critical, iconLevel: 5 },
          { name: "เตือนภัย",    value: stats.high,     color: COLOR.warning,  iconLevel: 4 },
          { name: "เฝ้าระวัง",   value: stats.low,      color: COLOR.watch,    iconLevel: 3 },
  ...
  /** Fetch rain — daily (24h accumulation) */
  ...
  /** Fetch rain — hourly (3h accumulation, existing endpoint) */
