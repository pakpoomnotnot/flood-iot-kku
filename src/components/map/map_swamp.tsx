...
function getActiveLegend(
  ...
  return { segments, activeIdx, freeboard };
}
...
const ChartModal: FC<ChartModalProps> = ({
  ...
  const lav = interpolateLAV(lakeId, currentLevel);
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-5 py-2.5 bg-blue-50/60 text-xs text-gray-500 border-b border-blue-100">
          ...
          {lakeData?.air_temp != null && (
            <span>🌡️ {lakeData.air_temp.toFixed(1)} °C</span>
          )}
          {lakeData?.air_humid != null && (
            <span>💧 ความชื้น {lakeData.air_humid.toFixed(0)}%</span>
          )}
          {lakeData?.rain_daily != null && (
            <span>🌧️ ฝนวันนี้ {lakeData.rain_daily.toFixed(1)} มม.</span>
          )}
          {lakeData?.wind_direction_name && (
            <span>💨 ลม {lakeData.wind_direction_name}</span>
          )}
...
const MapComponentSwamp: FC = () => {
  ...
  // ── FIX: ใช้ ref คู่กับ state เพื่อให้ closure ใน addStationMarkers อ่านค่าล่าสุดได้เสมอ
  const lakesDataRef = useRef<LakeApiItem[]>([]);

  // ── legend state: ใช้ ref ควบคู่เพื่อ sync ค่าให้ถูกต้องแม้ closure เก่า
  const [activeLakeId, setActiveLakeId] = useState<string | null>(null);
  const [activeLakeLevel, setActiveLakeLevel] = useState<number>(0);
  const activeLakeIdRef = useRef<string | null>(null);
  const activeLakeLevelRef = useRef<number>(0);
  ...
  // ── helper: อ่านจาก ref เสมอ → ปลอดภัยใน closure เก่า
  ...
  // ── FIX: setActiveLake อัปเดตทั้ง ref และ state พร้อมกัน
  const setActiveLake = (lakeId: string | null, level: number) => {
    activeLakeIdRef.current = lakeId;
    activeLakeLevelRef.current = level;
    setActiveLakeId(lakeId);
    setActiveLakeLevel(level);
  };

  // ── marker สี = threshold color ของบึง PW, type อื่นใช้สีเดิม
  const createMarkerElement = (stationId: string): HTMLDivElement => {
    ...
    const baseConfig = stationTypeConfig[prefix] || {
      color: "#6B7280",
      icon: ICONS.mapPin,
    };
    ...
    // ── สี marker: PW ใช้ threshold color, type อื่นใช้สีเดิม
    const markerColor =
      prefix === "PW" && lakeId
        ? getThresholdColor(lakeId, waterLevel)
        : baseConfig.color;
    ...
    el.innerHTML = `
      <div class="custom-marker-animated" style="--marker-color: ${markerColor}; --water-height: ${waterHeight}%;">
        ...
        <div class="marker-icon">${baseConfig.icon}</div>
  ...
  const createPopupContent = (station: Station): string => {
    ...
    // ── FIX: อ่านจาก lakesDataRef โดยตรงเพื่อให้ได้ข้อมูลล่าสุดเสมอ
                ...
                ${
                  lavInfo
                    ? [100, 75, 50, 25, 0]
                        .map(
                          (p) =>
                            `<div class="scale-line" style="bottom:${p}%"><span>${p}%</span></div>`,
                        )
                        .join("")
                    : [6, 5, 4, 3, 2, 1, 0]
                        .map(
                          (n) =>
                            `<div class="scale-line" style="bottom:${(n / 6) * 100}%"><span>${n}</span></div>`,
                        )
                        .join("")
                }
              </div>
  ...
  const addStationMarkers = () => {
    ...
    data.stationTypes.forEach((stationType) => {
      stationType.stations.forEach((station) => {
        ...
        popup.on("open", () => {
          // ── FIX: สร้าง popup content จาก lakesDataRef ล่าสุด (ไม่ใช่ closure เก่า)
          popup.setHTML(createPopupContent(station));

          // ── อัปเดต legend ผ่าน ref + state พร้อมกัน
          const lakeId = STATION_TO_LAKE[station.id] ?? null;
          const ld = getLakeData(station.id);
          setActiveLake(lakeId, ld?.water_level ?? 0);
        ...
        popup.on("close", () => {
          // ── FIX: เช็คว่า popup ที่ปิดคือ active อันปัจจุบัน ถ้าใช่ค่อย reset
          // ป้องกันกรณีเปิด popup ใหม่แล้ว close event ของอันเก่า fire ทีหลัง
          const thisLakeId = STATION_TO_LAKE[station.id] ?? null;
          if (activeLakeIdRef.current === thisLakeId) {
            setActiveLake(null, 0);
          }
        });
      });
    });
  ...
  const activeLegend = activeLakeId
    ? getActiveLegend(activeLakeId, activeLakeLevel)
    : null;
  ...
  return (
    <div className="relative w-full h-full bg-gray-900 font-sans rounded-xl overflow-hidden flex flex-col">
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-lg text-xs font-medium h-20 transition-all ${
                        currentStyle === key
                          ...
                          : "bg-gray-50/50 hover:bg-blue-100/80 text-gray-700"
                      }`}
                    >
      ...
      {/* ── Legend — dynamic freeboard per lake ── */}
        ...
        {activeLegend ? (
          <>
            <div className="flex w-full rounded-sm overflow-hidden border border-gray-300">
              {activeLegend.segments.map((seg, i) => {
                const isActive = i === activeLegend.activeIdx;
                return (
                  <div
                    key={i}
                    ...
                    style={{
                      ...
                      filter: isActive
                        ? "none"
                        : "brightness(0.80) saturate(0.55)",
                      outline: isActive ? "2px solid #1e3a5f" : "none",
                      outlineOffset: "-2px",
                      position: "relative",
                      zIndex: isActive ? 1 : 0,
                    }}
                  >
                    <span
                      ...
                      style={{ color: isActive ? "#1e3a5f" : "#4b5563" }}
                    >
                      {seg.range}
                    </span>
                  </div>
                ...
                return (
                  <div
                    ...
                    className={`text-center text-[10px] font-semibold ${
                      i < activeLegend.segments.length - 1
          ...
          <>
            <div className="flex w-full rounded-sm overflow-hidden border border-gray-200 bg-gray-50">
              <div className="flex-1 flex items-center justify-center py-2">
                <span className="text-[10px] text-gray-400 font-medium">
                  📍 เลือกบึงเพื่อดูระดับน้ำ
                </span>
              </div>
            </div>
            <div className="flex w-full mt-0.5">
              <div className="flex-1 text-center text-[10px] text-gray-300">
                —
              </div>
            </div>
          </>
      ...
      <style jsx global>{`
        ...
        .lav-chip.pct .pct-val {
          color: var(--pct-color, #16a34a);
        }
        .popup-extra-row {
          display: flex;
          flex-wrap: wrap;
        ...
        .swamp-chart-btn:hover {
          background: #dcfce7;
          transform: scale(1.1);
        }
        .swamp-chart-btn:active {
